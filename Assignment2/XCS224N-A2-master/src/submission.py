...
def sigmoid(x):
  ...
  ### START CODE HERE
  s =  1./(1 + np.exp(-x)) #it ranges from value 0 to 1 and its a non-linear activation function useful for logistic regression
  ### END CODE HERE
...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  ### to integer overflow.
  dot_value = np.dot(outside_vectors,center_word_vec)
  softmax_value = softmax(dot_value)
  loss = - np.log(softmax_value[outside_word_idx])
  
  y_hat = softmax_value
  y_hat[outside_word_idx] -= 1 # y^ -  y
  grad_center_vec = outside_vectors.T.dot(y_hat) # dot product of y^ - y and outsidevector
  y_hat_row_2dim = y_hat[:,np.newaxis] # convert row vector (N,) -> (N,1)
  grad_outside_vecs = y_hat_row_2dim.dot(np.array([center_word_vec])) # dot product of y^ - y and centervector
  ### START CODE HERE
  ### END CODE HERE
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  ### START CODE HERE
  index_centerword = word2ind[current_center_word]
  vec_centerword = center_word_vectors[index_centerword]
  indices_outsideword = [word2ind[i] for i in outside_words]
  
  #loop to call word2vecfunction that calculates loss,gradcenterword, gradoutsideword for every outsideword
  for index_outsideword in indices_outsideword:
        each_loss,each_gradCenter,each_gradOutside = word2vec_loss_and_gradient(vec_centerword,index_outsideword,outside_vectors,dataset)
        loss += each_loss
        grad_center_vecs[index_centerword] += each_gradCenter
        grad_outside_vectors += each_gradOutside
