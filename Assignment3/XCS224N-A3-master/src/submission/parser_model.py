...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden = nn.Linear(self.n_features * self.embed_size,self.hidden_size) # running linear function of n features * embed size and output of hidden size
        nn.init.xavier_uniform_(self.embed_to_hidden.weight) # initialising weights via xavier uniform
        self.dropout = nn.Dropout(self.dropout_prob)
        self.hidden_to_logits = nn.Linear(self.hidden_size,self.n_classes) # calling Linear again from hidden size to n classes
